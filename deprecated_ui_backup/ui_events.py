"""
UI Events Module for VezylTranslator
Handles UI event management and coordination
"""
...
class UIEvents:
    ...
    def create_search_handler(self, search_var, render_callback, delay=300):
        ...
        def on_search_change(*args):
            if hasattr(on_search_change, 'after_id') and on_search_change.after_id:
                # Cancel previous timer
                try:
                    search_var.tk.after_cancel(on_search_change.after_id)
                except:
                    pass
            
            # Set new timer
            on_search_change.after_id = search_var.tk.after(delay, render_callback)
        
        on_search_change.after_id = None
